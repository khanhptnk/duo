...
import duo_ai
from duo_ai.models.ppo import PPOModelOutput
...
def parse_args():
    parser = argparse.ArgumentParser(description="Duo training script")
    parser.add_argument(
        "--config",
        type=str,
        default=None,
...
def make_coord_envs(base_envs, config):
    novice = duo_ai.load_policy(config.train_novice, base_envs["train"])
    expert = duo_ai.load_policy(config.train_expert, base_envs["train"])
...
def evaluate(args, config):
    ...
    if config.policy.load_path is None:
        ...
        policy = duo_ai.load_policy(config.policy.load_path, envs["train"])
    for split in splits:
        logging.info(f"Evaluating on {split} split")
        duo_ai.Evaluator(config.evaluation, envs[split]).evaluate(policy)


def visualize(args, config):
    base_env = make_base_env(config.env, "test_hard", render_mode="human")
    if args.type == "coord":
        novice = duo_ai.load_policy(config.train_novice, base_env)
        expert = duo_ai.load_policy(config.train_expert, base_env)
        env = duo_ai.CoordEnv(config.coordination, base_env, novice, expert)
        env.set_costs(0.05)
    else:
        env = base_env
    policy = duo_ai.load_policy(config.policy.load_path, env)
...
def main():
    # NOTE: register the MiniGrid configuration with Duo
